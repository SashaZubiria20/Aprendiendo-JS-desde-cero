...
// formula = console.log(Math.round(Math.random() * (max-min)+min));
// formula correcta = Math.floor(Math.random() * (max - min + 1)) + min;
...
let areaPizzaChica = Math.floor(areaPizza);
console.log(areaPizzaChica);
let areaPizzaGrande = Math.ceil(areaPizza);
console.log(areaPizzaGrande);
let areaPizzaMediana = Math.round(areaPizza);
console.log(areaPizzaMediana);
let precio = 15.45;
let mensaje = `Contamos con una promocion de 3 Pizzas por un valor de ${precio}: Incluye 1 Pizza chica ${areaPizzaChica}, 1 Pizza mediana ${areaPizzaMediana} y 1 Pizza grande ${areaPizzaGrande}`;
